...
public class Weapon : MonoBehaviour
{
    ...
    private void SpawnBullet(Vector3 position, Quaternion quaternionRotation)
    {
        var bulletPrefab = Instantiate(weaponData.BulletData.bulletPrefab, position, quaternionRotation);
        bulletPrefab.GetComponent<Bullet>().BulletData = weaponData.BulletData;
    }

    private Quaternion CalculateAngel(GameObject muzzle)
    {
        float spread = Random.RandomRange(-weaponData.SpreadAngel, weaponData.SpreadAngel);
